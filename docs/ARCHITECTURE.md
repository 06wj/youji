...
`PlayInsightCalculator` 已把这些差值用于“游玩洞察”，同时输出按游戏拆分的时长与奖杯贡献供周期详情使用。没有足够同步时间点时展示零变化并解释数据如何积累，不会用累计总量冒充区间增量。年度月度分布按同步快照落点计算；跨月未同步的累计变化会归入下一次同步所在月份，并在界面明确说明。
...
- `DashboardView`：总览与平台视图、完整跨平台游戏库、排序和同步入口；洞察与游戏大脑以小型胶囊入口嵌入总览卡，待玩清单位于更多菜单，同步回执留在平台页，Switch 游玩日历只出现在 Switch 视图。
- `GameDetailView`：单款游戏时间线、奖杯/每日记录、收藏、隐藏、状态、私人备注，以及加入当前账号范围待玩清单的行动入口。
- `AccountCenterView`：当前连接身份、分平台同步时间、同步变化回执和离线历史账号档案。
- `InsightsView` / `SavedPlansView`：7 天、30 天、年度快照变化和同步日覆盖说明；洞察周期支持下钻到月度、游戏/奖杯贡献和单款档案；行动清单区分待行动/已完成并防止整行误触。
...
- `SettingsView`：从首页进入时优先展示产品与数据，从 AI 页面进入时优先展示模型配置；支持保存/测试 AI 服务，并管理同步提醒、数据和隐私入口。
...
完整 JSON 备份采用版本化 `YouJiBackup`，包含游戏、自定义字段、同步快照、每日活动、AI 会话、人格历史和清单。恢复采用按唯一标识 upsert，备份中的同一记录更新本机值，备份外的本机记录不删除。Keychain 和 UserDefaults 中的平台凭据、AI API Key、当前身份与同步状态不进入备份。若某个平台没有当前身份但只有一个本地历史账号，首页会自动展示该档案；存在多个历史账号时仍需在账号中心明确选择，避免混合不同账号。“删除全部本地档案”同时清除 SwiftData、封面缓存、账号显示身份和同步状态，但按界面说明保留 Keychain 凭据，供用户再次主动同步或单独清除。
